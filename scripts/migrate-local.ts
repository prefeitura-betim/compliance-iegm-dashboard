...
async function createTables() {
  console.log('🏗️  Criando tabelas no Cloudflare D1 (local)...');

  try {
    const command = 'wrangler d1 execute DB --file=scripts/create-d1-tables.sql --local';
...
async function clearD1Tables(): Promise<void> {
  console.log('🧹 Limpando tabelas existentes no D1 (local)...');
  ...
  try {
    const command = `wrangler d1 execute DB --file=${sqlFilename} --local`;
    execSync(command, { stdio: 'inherit' });
...
async function importCSVToD1(filename: string, tableName: string): Promise<number> {
  try {
    console.log(`Importando ${filename} para D1 (local)...`);
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      try {
        ...
        // Usar wrangler para executar o arquivo SQL (local)
        const command = `wrangler d1 execute DB --file=${sqlFilename} --local`;
        execSync(command, { stdio: 'inherit' });
...
async function main() {
  ...
  try {
    ...
    console.log('  2. Testar localmente: yarn dev:d1');
    console.log('  3. Para produção: yarn migrate:prod');
...
export { main as migrateToD1Local };
