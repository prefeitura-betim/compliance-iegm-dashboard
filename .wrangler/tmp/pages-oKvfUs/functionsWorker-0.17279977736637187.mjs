...
// ../.wrangler/tmp/bundle-lNVRUx/checked-fetch.js
...
__export(schema_exports, {
  ...
  resultadosEstados: () => resultadosEstados,
  resultadosIndicadores: () => resultadosIndicadores,
  resultadosMunicipios: () => resultadosMunicipios,
  tribunais: () => tribunais
});
var tribunais = sqliteTable("tribunais", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  codigo: text("codigo").notNull().unique(),
  // ex: TCEMG
  nome: text("nome").notNull(),
  // ex: Tribunal de Contas do Estado de Minas Gerais
  uf: text("uf").notNull()
  // ex: MG
}, (table) => ({
...
var municipios = sqliteTable("municipios", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var indicadores = sqliteTable("indicadores", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  codigo: text("codigo").notNull().unique(),
  // ex: i-Amb, i-Cidade, i-Educ
  nome: text("nome").notNull(),
  // ex: Indicador de Meio Ambiente
...
var questionarios = sqliteTable("questionarios", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var questionarioRespostas = sqliteTable("questionario_respostas", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  tribunalId: integer("tribunal_id").notNull().references(() => tribunais.id),
  municipioId: integer("municipio_id").notNull().references(() => municipios.id),
  questionarioId: integer("questionario_id").notNull().references(() => questionarios.id),
  questionarioRespostaId: integer("questionario_resposta_id").notNull(),
  // ID original
...
var respostas = sqliteTable("respostas", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  ...
  chaveResposta: text("chave_resposta"),
  // ex: "M05Q00900R00100"
...
var resultadosIndicadores = sqliteTable("resultados_indicadores", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  tribunalId: integer("tribunal_id").notNull().references(() => tribunais.id),
  municipioId: integer("municipio_id").notNull().references(() => municipios.id),
  indicadorId: integer("indicador_id").notNull().references(() => indicadores.id),
...
var resultadosMunicipios = sqliteTable("resultados_municipios", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  tribunalId: integer("tribunal_id").notNull().references(() => tribunais.id),
  municipioId: integer("municipio_id").notNull().references(() => municipios.id),
  anoRef: integer("ano_ref").notNull(),
...
var resultadosEstados = sqliteTable("resultados_estados", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  tribunalId: integer("tribunal_id").notNull().references(() => tribunais.id),
  uf: text("uf").notNull(),
  anoRef: integer("ano_ref").notNull(),
...
var respostasDetalhadas = sqliteTable("respostas_detalhadas", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  ...
  resposta: text("resposta").notNull(),
  // ex: "Adequado", "Necessita Melhoria"
  pontuacao: real("pontuacao"),
  // percentual (0-100)
  peso: real("peso").default(1),
  nota: real("nota"),
  // pontuação na escala 0-1000
  anoRef: integer("ano_ref").notNull()
}, (table) => ({
...
var migracoes = sqliteTable("migracoes", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  ...
  // 'calculo', 'geral_municipio', 'geral_estado', 'respostas'
  anoRef: integer("ano_ref"),
  tribunal: text("tribunal")
});

// api/[[route]].ts
function createD1Connection(d1) {
  return drizzle(d1, { schema: schema_exports });
}
__name(createD1Connection, "createD1Connection");
var corsHeaders = {
...
async function handleMunicipios(request, db, url) {
  ...
  const whereConditions = [
    ...
    eq(resultadosMunicipios.tribunalId, 2)
    // TCEMG
  ];
  if (municipio) {
    whereConditions.push(like(municipios.nome, `%${municipio}%`));
  }
  ...
  }).from(resultadosMunicipios).innerJoin(municipios, eq(resultadosMunicipios.municipioId, municipios.id)).where(and(...whereConditions)).orderBy(desc(resultadosMunicipios.percentualIegmMunicipio)).limit(limit).offset(offset);
  return new Response(JSON.stringify(results), {
    headers: { ...corsHeaders, "Content-Type": "application/json" }
  });
}
__name(handleMunicipios, "handleMunicipios");
async function handleRanking(request, db, url) {
  const ano = url.searchParams.get("ano");
  let tribunal = url.searchParams.get("tribunal");
  const limit = parseInt(url.searchParams.get("limit") || "1000");
  const offset = parseInt(url.searchParams.get("offset") || "0");
  ...
  const results = await db.select({
    ...
    percentualIegmMunicipio: resultadosMunicipios.percentualIegmMunicipio,
    faixaIegmMunicipio: resultadosMunicipios.faixaIegmMunicipio
  }).from(resultadosMunicipios).innerJoin(municipios, eq(resultadosMunicipios.municipioId, municipios.id)).where(
    and(
      eq(resultadosMunicipios.anoRef, parseInt(ano)),
      eq(resultadosMunicipios.tribunalId, 2)
      // TCEMG
    )
  ).orderBy(desc(resultadosMunicipios.percentualIegmMunicipio)).limit(limit).offset(offset);
  const rankingResults = results.map((item, index2) => ({
    ...item,
    ranking: index2 + 1 + offset,
...
async function handleEstatisticas(request, db, url) {
  const ano = url.searchParams.get("ano");
  let tribunal = url.searchParams.get("tribunal");
  if (!tribunal) tribunal = "TCEMG";
  if (!ano) {
  ...
  }).from(resultadosMunicipios).where(
    and(
      eq(resultadosMunicipios.anoRef, parseInt(ano)),
      eq(resultadosMunicipios.tribunalId, 2)
      // TCEMG
...
__name(handleEstatisticas, "handleEstatisticas");
async function handleFaixasDistribuicao(request, db, url) {
  const ano = url.searchParams.get("ano");
  if (!ano) {
    return new Response(JSON.stringify({ error: "Missing required parameter: ano" }), {
      status: 400,
  ...
  ).groupBy(resultadosMunicipios.faixaIegmMunicipio).orderBy(asc(resultadosMunicipios.faixaIegmMunicipio));
  return new Response(JSON.stringify(faixas), {
    headers: { ...corsHeaders, "Content-Type": "application/json" }
  });
}
__name(handleFaixasDistribuicao, "handleFaixasDistribuicao");
async function handleAnaliseDimensoes(request, db, url) {
  const municipio = url.searchParams.get("municipio");
  const ano = url.searchParams.get("ano");
  if (!municipio || !ano) {
  ...
  const municipioData = await db.select().from(resultadosMunicipios).innerJoin(municipios, eq(resultadosMunicipios.municipioId, municipios.id)).where(
    and(
      eq(municipios.nome, municipio),
      eq(resultadosMunicipios.anoRef, parseInt(ano))
    )
  ).limit(1);
  if (!municipioData[0]) {
  ...
  const dimensoes = [
    { nome: "Educa\xE7\xE3o", score: data.resultados_municipios.percentualIeduc || 0 },
    { nome: "Sa\xFAde", score: data.resultados_municipios.percentualIsaude || 0 },
    { nome: "Gest\xE3o Fiscal", score: data.resultados_municipios.percentualIfiscal || 0 },
    ...
    { nome: "Governan\xE7a TI", score: data.resultados_municipios.percentualIgovTi || 0 }
  ];
  return new Response(JSON.stringify(dimensoes), {
    headers: { ...corsHeaders, "Content-Type": "application/json" }
  });
}
__name(handleAnaliseDimensoes, "handleAnaliseDimensoes");
async function handleRespostasDetalhadas(request, db, url) {
  const ano = url.searchParams.get("ano");
  let tribunal = url.searchParams.get("tribunal");
  const municipio = url.searchParams.get("municipio");
  ...
  const whereConditions = [
    ...
    eq(questionarioRespostas.tribunalId, 2)
    // TCEMG
  ];
  if (municipio) {
    whereConditions.push(eq(municipios.nome, municipio));
  }
  if (indicador) {
  ...
  }).from(respostas).innerJoin(questionarioRespostas, eq(respostas.questionarioRespostaId, questionarioRespostas.id)).innerJoin(municipios, eq(questionarioRespostas.municipioId, municipios.id)).innerJoin(questoes, eq(respostas.questaoId, questoes.id)).innerJoin(questionarios, eq(questoes.questionarioId, questionarios.id)).innerJoin(indicadores, eq(questionarios.indicadorId, indicadores.id)).where(and(...whereConditions)).orderBy(desc(respostas.nota)).limit(1e3);
  return new Response(JSON.stringify(results), {
    headers: { ...corsHeaders, "Content-Type": "application/json" }
  });
}
__name(handleRespostasDetalhadas, "handleRespostasDetalhadas");
async function handleComparativoAnoAnterior(request, db, url) {
  const municipio = url.searchParams.get("municipio");
  ...
  const dadosAtual = await db.select({
    percentualIegmMunicipio: resultadosMunicipios.percentualIegmMunicipio
  }).from(resultadosMunicipios).innerJoin(municipios, eq(resultadosMunicipios.municipioId, municipios.id)).where(
    and(
      eq(municipios.nome, municipio),
  ...
  const dadosAnterior = await db.select({
    percentualIegmMunicipio: resultadosMunicipios.percentualIegmMunicipio
  }).from(resultadosMunicipios).innerJoin(municipios, eq(resultadosMunicipios.municipioId, municipios.id)).where(
    and(
      eq(municipios.nome, municipio),
...
__name(handleComparativoAnoAnterior, "handleComparativoAnoAnterior");

// ../.wrangler/tmp/pages-oKvfUs/functionsRoutes-0.05755553528467949.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// ../../../../../.nvm/versions/node/v22.14.0/lib/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// ../../../../../.nvm/versions/node/v22.14.0/lib/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
...
// ../.wrangler/tmp/bundle-lNVRUx/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-lNVRUx/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.17279977736637187.mjs.map
